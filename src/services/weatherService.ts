export default { 
    ...
    async get5daysGeolocation(lat: string, lon: string) {
       const apiCall = await fetch(`https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&lang=fr&units=metric&appid=1be2d16ca1e8f4250b426a94cbb888ca`)
       const data = await apiCall.json()
       console.log(data)
       return data;
    }
